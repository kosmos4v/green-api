...
export const chatReducer = handleActions<TChatState, TChatAction>({
  ...
  [ADD_NEW_CHAT]: (state, { payload }) => {
    ...
    const chats = {
      ...state.chats,
      [phone]: { phone, chatId: `${phone}@c.us`, messages: [] },
    };
    return ({
      ...state,
      chats,
  ...
  [RECEIVE_NOTIFICATION_SUCCESS]: (state, { payload }) => {
    ...
    && (
      ...
      const { chatId } = payload.notification.body.senderData;
    ...
    return ({
      ...
      receiptId: payload.notification?.receiptId,
      isNotificationRendered: true,
      // для удаления сообщений имеющих другой typeMessage нежели 'textMessage' и 'quotedMessage'
    });
  },
